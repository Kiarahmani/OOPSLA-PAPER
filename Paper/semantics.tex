...
\begin{array}{lclcl}
  ...
  \eff & \in & \mathtt{Effect} & \coloneqq &  (s,i,op,v)\\
   ...
   {\tagger} & \in & \mathtt {Tagger} & \coloneqq & \eta \mapsto \{0,1,2\} \\
  ...
  \Theta  & \in & \mathtt{Store}      & \coloneqq & \rho \mapsto (\Pool,\Cache,\tagger) \\
...
{ \\
  ...
  \eta \not\in \Pool \\
  \Pool' = \Pool \cup \{\eta\} \spc
  \Theta' = \Theta[\rho \mapsto (\Pool',\Cache,\tagger)]\\
}
{
...
\rulelabel{Tag Refresh}
\begin{smathpar}
\stretcharraybig
\begin{array}{l}
\RuleTwo
{
  \Theta(\rho) = (\Pool,\Cache,\tagger) \spc \eta \in \Pool
  \\ \tagger(\eta) < i \spc
  \tagger' = \tagger [\eta \mapsto i] \\ 
  \psi_i^{-1} (\eta) \subseteq \Pool
 \spc  \Theta' = \Theta[\rho \mapsto (\Pool,\Cache,\tagger')]\\
}
{
  (\E,\Theta,\Sigma) \;\xrightarrow{\eff}\; (\E, \Theta', \Sigma)
}
...
\RuleTwo
{
  \Theta(\rho) = (\Pool,\Cache,\tagger) \spc \eta \in \Pool
  \\ \eta \not\in \Cache \spc
  \Cache'=\Cache \cup \{\eta\} \\ \psi^{-1} (\eta) \subseteq \Cache
 \spc  \Theta' = \Theta[\rho \mapsto (\Pool,\Cache',\tagger)]\\
}
{
  (\E,\Theta,\Sigma) \;\xrightarrow{\eff}\; (\E, \Theta', \Sigma)
