...
strict
%\footnote{The only difference is an
%extra conjunct, ensuring the presence of mid-level effects before each recursive call} 
...
%hold for $V=\{\eta_1,\eta_6,\eta_7,\eta_9\}$
...
And finally, we define \emph{closed subsets} of a given set $V$, 
...
%--- The Aux [OPER] rule
The rule
\rulelabel{oper} represtns the procedure of generating a new effect $\eff$, by witnessing a set
...
starting with \rulelabel{ub exec}, which represents the execution of an
operation
